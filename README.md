...
- 母线电流重建值与(可选的)直流侧趋磁趋势矛盾/持续为零带转矩指令 → 桥臂开路/失效诊断;
...
- **架构理由(buck→boost 的必要性)**:TPS61170(升压至 10 V)的输入上限仅 **18 V**,若从电池(6S = 25.2 V)直接升压会超限;先经 SY8301 把 5.5~25.2 V 归一化为 5 V,使 boost 输入恒 ≤5 V,避开 18 V 上限,5 V→10 V 也恰为其甜点区。
...
- **电池欠压切除(必设)**:SY8301 守住 5 V 需输入 ≥ ~5.2 V,ME6211 保住 3.3 V 需输入 ≥ ~3.5 V。电池/软件 UVLO 端**建议设在 ~6.0 V(2S)及以上**,防止 MCU 在边沿态工作。
...
| 保护 | 实现 | 响应 |
|---|---|---|
| 过流(硬件级,主保护) | STM32G4 **COMP 比较 INA240 输出 vs DAC 阈值**(每相或至少两相)→ TIM1 **BRK** | ns~µs 级硬件关断 PWM,不依赖软件 |
...
> 尤其推荐把 **COMP → TIM 刹车** 作为主保护:即使 MCU 主循环或电流环异常,也能在定时器硬件层面快速关断三相 PWM,防止上/下桥直通烧毁器件。G473 的 COMP 非反相端走 INA240 输出引脚、反相端接内部 DAC,阈值(对应 ±55 A 左右)软件可调。
