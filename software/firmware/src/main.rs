...
use foc_firmware::bsp_hardware::{
    BoardIrqs, Tim1PwmBridge, init_injected_adcs, wait_for_adc_frame,
};
...
async fn main(spawner: Spawner) {
    ...
    init_injected_adcs(
        p.ADC1, p.ADC2, p.ADC3, p.PA0, p.PA1, p.PB0, p.PA2, p.PA3,
    );

    let foc_config = FocConfig::default();
    #[cfg(all(feature = "sensor-encoder", not(feature = "sensor-hall")))]
    let sensor =
        EncoderSensor::new(p.TIM3, p.PB4, p.PB5, ENCODER_COUNTS_PER_REVOLUTION);
    #[cfg(all(feature = "sensor-hall", not(feature = "sensor-encoder")))]
    let sensor =
    ...
    spawner
        .spawn(motor_control_task(pwm, sensor, foc_config))
        .unwrap();
